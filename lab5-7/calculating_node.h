...
class CalculationNode {
    ...
    public:
		...
		std::chrono::time_point<std::chrono::_V2::steady_clock, std::chrono::duration<long int, std::ratio<1, 1000000000> > > start_time;
        ...
        std::string exec (std::string str) {
			...
			if(str == "stop"){
				...
				//std::cout << diff;
				duration += (unsigned long long)(diff);
				start_time = std::chrono::steady_clock::now();
			}
			...
			if(str == "time"){
				...
				std::cout << diff;
				duration += (unsigned long long)(diff);
            	std::string answer = "Ok: " + std::to_string(id) + " " + std::to_string(duration);
